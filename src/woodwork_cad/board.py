...
Vector3d_ZERO = Vector3d(0.0, 0.0, 0.0)
...
class Size:
    ...
    def expand(self, length: float = 0.0, width: float = 0.0, depth: float = 0.0) -> "Size":
        return Size(self.length + length, self.width + width, self.depth + depth)

    def contract(self, length: float = 0.0, width: float = 0.0, depth: float = 0.0) -> "Size":
        return Size(self.length - length, self.width - width, self.depth - depth)
...
class Board:
    ...
    def rotate(self, rotate_x: float = 0.0, rotate_y: float = 0.0, rotate_z: float = 0.0) -> None:
    ...
    def cut(self, length: float, kerf: float = 5, angle: float = 90):
        ...
        self.add_cut("cut", length, 0, length + kerf + dx, self.W, L=length, angle=angle)
        return [
            Board(
                length,
                self.W,
                ...
                _profile=self.profile.cut(length, 0, length + dx, self.W, length, self.T, right=False),
            ),
            Board(
                self.L - length - kerf,
                self.W,
    ...
    def rip(self, width: float, kerf: float = 5):
        ...
        self.add_cut("rip", x1(width, 0), width, x2(width + kerf, 0), width + kerf, W=width)
        return [
            Board(
                self.L,
                width,
                ...
                _profile=self.profile.rip(0, width, self.L, width, self.L, self.T, bottom=False),
            ),
            Board(
                self.L,
                self.W - width - kerf,
                ...
                _profile=self.profile.rip(0, width + kerf, self.L, width + kerf, self.L, self.T, bottom=True),
            ),
        ]
    ...
    def dovetail_tails(
        ...
        *,
        ...
        self.dovetails.add_tails(tails, self.L, self.W, self.T, base, angle, right=right)
    ...
    def dovetail_pins(
        ...
        *,
        ...
        self.dovetails.add_pins(tails, self.L, self.W, self.T, base, angle, right=right)
    ...
    def draw_board(
        ...
        if False:
            canvas.polyline3d(
                "magenta",
                [Point3d(p.x, p.y, 0) for p in self.profile._shape],  # noqa: SLF001
                x,
                y,
                closed=True,
            )
            for p in self.profile._shape:  # noqa: SLF001
    ...
    def rotated_faces(
        ...
        faces.extend(rotate_faces(self.get_faces(x1, x2), origin, rotate_y, offset, mate, origin2d))

        return origin2d[0], origin2d[1]

    def get_faces(self, x1: Interpolator, x2: Interpolator) -> Iterable[Face]:
        ...
        yield from self.dovetails.left_right(x2, self._get_left_right(x2).reverse(), right=True)
        ...
        for side in sides:
            yield (self._get_top_bottom(x1, x2, side).remove_side(self.dovetails.sides(x1, x2)).reverse())
        ...
        for side in sides:
            yield self._get_top_bottom(x1, x2, side).remove_side(self.dovetails.sides(x1, x2))
    ...
    def _get_back(
        ...
        *,
        ...
        for flat in sorted(self.grooves.flats(self.W, self.T, face=False), reverse=True):
    ...
    def _get_shade_right(self, x1: Interpolator, x2: Interpolator) -> Iterator[Face]:
        if self.dovetails:
            for face_clip in self._get_shade_right_clip(x1, x2):
                face_clip.points = peturb(face_clip.points)
                x = face_clip.points[0].x - x2(face_clip.points[0].y, 0) - x1(face_clip.points[0].y, 0)
    ...
    def _draw_cuts(self, canvas: SVGCanvas, x: float, y: float) -> None:
        ...
        for cut in self.cuts:
            if cut.op:
                ...
                canvas.polyline3d(cut.colour, points, x=x, y=y, fill=cut.fill, closed=True)
            ...
            if cut.label:
                canvas.text(x + cut.lx, y + cut.ly, "start", content=cut.label, style="")
    ...
    def get_dimension(
        ...
        if dimension == "W" and position == "left":
        ...
        if dimension == "L" and position == "below":
        ...
        if dimension == "L" and position == "above":
        ...
        if dimension == "T" and position == "below":
            corner_left = Point3d(0, self.W, 0)
        ...
        if dimension == "T" and position == "above":
